...
namespace valhalla {
  namespace loki {
    ...
    void loki_worker_t::parse_costing(rapidjson::Document& request) {
      ...
      try{
        //Armin H. modified to allow graph reader for autocost
        cost_ptr_t c = factory.Create(*costing, *method_options_ptr, reader);
        //OLD: c = factory.Create(*costing, *method_options_ptr); 
        //Armin H.    
