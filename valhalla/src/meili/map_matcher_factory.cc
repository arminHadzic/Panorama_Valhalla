...
MapMatcherFactory::Create(const std::string& costing, const boost::property_tree::ptree& preferences)
{
  ...
  //Armin H.
  // OLD:  valhalla::sif::cost_ptr_t cost = get_costing(config, costing)
  valhalla::sif::cost_ptr_t cost = get_costing(config, costing, graphreader_);
...
/*
Armin H. all costs have graph readers.
//OLD
...
sif::cost_ptr_t MapMatcherFactory::get_costing(const boost::property_tree::ptree& request,
      ...
      std::string method_options = "costing_options." + costing;
      auto costing_options = request.get_child(method_options, {});
      return cost_factory_.Create(costing, costing_options, graphreader);
}
