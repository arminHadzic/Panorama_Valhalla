...
//Armin H. custom get_costing for Auto
//OLD:
/*
...
valhalla::sif::cost_ptr_t get_costing(CostFactory<DynamicCost> factory,
  ...
  std::string method_options = "costing_options." + costing;
  auto costing_options = request.get_child(method_options, {});
  return factory.Create(costing, costing_options, graphreader);
}
//Armin H.
...
int main(int argc, char *argv[]) {
  ...
  if (routetype == "multimodal") {
    ...
    //Armin H. Custom costing for auto to pass in the graph reader.
    // OLD: mode_costing[0] = get_costing(factory, json_ptree, "auto");
    mode_costing[0] = get_costing(factory, json_ptree, "auto", reader);
    mode_costing[1] = get_costing(factory, json_ptree, "pedestrian", reader);
    mode_costing[2] = get_costing(factory, json_ptree, "bicycle", reader);
    mode_costing[3] = get_costing(factory, json_ptree, "transit", reader);
    //Armin H.
    ...
    //Armin H. modified costing to accomodate auto costing getting a graph reader reference.
    std::shared_ptr<DynamicCost> cost = get_costing(factory,
                              json_ptree, routetype, reader);
    //OLD: std::shared_ptr<DynamicCost> cost = get_costing(factory, json_ptree, routetype);
    //Armin H.
