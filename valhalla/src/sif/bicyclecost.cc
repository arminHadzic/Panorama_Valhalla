...
class BicycleCost : public DynamicCost {
 public:
   ...
   */
  //Armin H. Modified to make all costs take a graph reader.
  BicycleCost(const boost::property_tree::ptree& config, baldr::GraphReader& graphreader);
...
protected:
  ...
  /**
    Armin:
    private variable to hold the tree
  **/
  const boost::property_tree::ptree& m_tree_cpy;
...
// Constructor
BicycleCost::BicycleCost(const boost::property_tree::ptree& pt, baldr::GraphReader& graphreader)
    : DynamicCost(pt, graphreader, TravelMode::kBicycle), m_tree_cpy(pt),
...
Cost BicycleCost::EdgeCost(const baldr::DirectedEdge* edge) const {
  ...
  *
  ...
  std::cout << "The total stress is: " << total_stress <<std::endl;
  GraphId the_endNode = edge->endnode();
  std::cout << "Line 1" << std::endl;
  baldr::GraphReader lcl_GraphReader(m_tree_cpy);
  ...
  if((lcl_lat >= 38.017f) && (lcl_lng <= -84.465f)) {
    total_stress = total_stress * 20.0f;
    std::cout << "BINGO: The total stress is: " << total_stress <<std::endl;
  }
  std::cout << "The total stress is: " << total_stress <<std::endl;
...
Cost BicycleCost::TransitionCost(const baldr::DirectedEdge* edge,
  ...
  */
  ...
  std::cout << "The total transition stress is: " << turn_stress <<std::endl;
  float lcl_lat = node->latlng().lat();
  float lcl_lng = node->latlng().lng();

  if((lcl_lat >= 38.017f) && (lcl_lng <= -84.465f)) {
    turn_stress = turn_stress * 20.0f;
    std::cout << "BINGO: The total transition stress is: " << turn_stress <<std::endl;
  }
  std::cout << "The total transition stress is: " << turn_stress <<std::endl;
...
BicycleCost* make_bicyclecost_from_json(const std::string& property, float testVal) {
  ...
  return new BicycleCost(costing_ptree, reader);
}
