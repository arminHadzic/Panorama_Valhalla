...
class TruckCost : public DynamicCost {
 public:
   ...
   */
  //Armin H. Modified to include graphreader
  TruckCost(const boost::property_tree::ptree& config, baldr::GraphReader& graphreader);
  //Armin H.
...
// Constructor
TruckCost::TruckCost(const boost::property_tree::ptree& pt, baldr::GraphReader& graphreader)
    : DynamicCost(pt, graphreader, TravelMode::kDrive),
...
TruckCost* make_truckcost_from_json(const std::string& property, float testVal) {
  ...
  return new TruckCost(costing_ptree, reader);
}
