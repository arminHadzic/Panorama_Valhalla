...
struct Coordinate
{
  ...
  Coordinate():lng(1.0), lat(1.0), score(1.0) {}
};

bool float_approx_equals(float a, float b, float epsilon);
void log_csv(float lat, float lng);

bool float_approx_equals(float a, float b, float epsilon = 0.00001)
{
...
void log_csv(float lat, float lng)
{
  ...
  storage_csv.precision(17);
  std::string file_loc = "../results/route_log.csv";
  storage_csv.open(file_loc.c_str(), std::ofstream::app);
...
class AutoCost : public DynamicCost {
 public:
   ...
   */
  //Armin H. Modified constructor to pass in the graph reader
  AutoCost(const boost::property_tree::ptree& config, baldr::GraphReader& graphreader);
 ...
 public:
  ...
  //Armin
  //Imported edges that are being evaluated.
  std::vector<Coordinate> graded_edges;
  baldr::GraphReader& lcl_GraphReader;
  //Armin

};


//Armin: modified constructor to pass in graph reader reference
// Constructor
AutoCost::AutoCost(const boost::property_tree::ptree& pt, baldr::GraphReader& graphreader)
    : DynamicCost(pt, graphreader, TravelMode::kDrive), lcl_GraphReader(graphreader),
  ...
  //Armin
  // Read scores from CSV once
  std::string filename = "../predictions/network_predictions.csv";
  ...
  if(myfile)
  {
    ...
    while(getline(myfile, current_line))
    {
      if(cnt > 0)
      {
        ...
        tmp_coordinate.lat = std::stof(value);
        getline(linestream, value, ',');
        tmp_coordinate.lng = std::stof(value);
        getline(linestream, value, ','); //prediction line, we can toss it
        getline(linestream, value, ','); //Score 1 line, we can toss it
        getline(linestream, value, ','); //Score 2 line, we can toss it
        getline(linestream, value, ','); //Score 3 line, we can toss it
        getline(linestream, value, ','); //Score 4 line, we can toss it
        getline(linestream, value, ','); //Score 5 line, we can toss it
        getline(linestream, value, ','); //Adjusted score line, this is the keeper
        tmp_coordinate.score = std::stof(value);
        graded_edges.push_back(tmp_coordinate);   
        //we ignore the rest of the columns
      }
      else
        cnt++;
    }
  ...
  //Create log file for storing coordinates in route.
  std::ofstream storage_csv;
  std::string file_loc2 = "../results/route_log.csv";
  storage_csv.open(file_loc2.c_str(), std::ofstream::trunc);
...
Cost AutoCost::EdgeCost(const DirectedEdge* edge) const {
  ...
  Modifying edge cost!
  */
  GraphId the_endNode = edge->endnode();
  const PointLL& lcl_latlng = (lcl_GraphReader.nodeinfo(the_endNode))->latlng();
  float lcl_lat = lcl_latlng.lat();
  float lcl_lng = lcl_latlng.lng();

  log_csv(lcl_lat, lcl_lng); //Log the lat and lng coordinates of the route for future use

  for(int i = 0; i < graded_edges.size(); i++)
  {
    if(float_approx_equals(graded_edges[i].lat, lcl_lat, 0.00001) && float_approx_equals(graded_edges[i].lng, lcl_lng, 0.00001))
    {
      factor = factor * graded_edges[i].score;
      std::cout << "FOUND ONE: The total road quality is: " << factor <<std::endl;
      break;
    }
  }
  /*
...
Cost AutoCost::TransitionCost(const baldr::DirectedEdge* edge,
  ...
  /**
  * Armin: Modify the edge cost here
  *
  */

  float lcl_lat = node->latlng().lat();
  float lcl_lng = node->latlng().lng();
  float road_quality = 1.0f;
  ...
  for(int i = 0; i < graded_edges.size(); i++)
  {
    if(float_approx_equals(graded_edges[i].lat, lcl_lat, 0.00001) && float_approx_equals(graded_edges[i].lng, lcl_lng, 0.00001))
    {
      road_quality = road_quality * graded_edges[i].score;
      std::cout << "FOUND ONE: The total road quality is: " << road_quality <<std::endl;
      break;
    }
  }
  /*
  ...
  */
  ...
  return { seconds*road_quality + penalty, seconds };
}
...
//Armin H. modified to have graph reader argument
cost_ptr_t CreateAutoCost(const boost::property_tree::ptree& config, baldr::GraphReader& graphreader) {
  return std::make_shared<AutoCost>(config, graphreader);
}
...
class AutoShorterCost : public AutoCost {
 public:
   ...
   * @param  config  Property tree with configuration/options.
   */
  //Armin H. modified to match autocost constructor
  AutoShorterCost(const boost::property_tree::ptree& config, baldr::GraphReader& graphreader);
...
// Constructor
//Armin H. modified constructor to match AutoCost
AutoShorterCost::AutoShorterCost(const boost::property_tree::ptree& pt, baldr::GraphReader& graphreader)
    : AutoCost(pt, graphreader) {
...
cost_ptr_t CreateAutoShorterCost(const boost::property_tree::ptree& config, baldr::GraphReader& graphreader) {
  return std::make_shared<AutoShorterCost>(config, graphreader);
}
...
class BusCost : public AutoCost {
 public:
   ...
   */
  //Armin H. Modified constructor to match autocost
  BusCost(const boost::property_tree::ptree& config, baldr::GraphReader& graphreader);
...
//Armin H. modified constructor to match autocost constructor change
// Constructor
BusCost::BusCost(const boost::property_tree::ptree& pt, baldr::GraphReader& graphreader)
    : AutoCost(pt, graphreader) {
  type_ = VehicleType::kBus;
}
...
class HOVCost : public AutoCost {
 public:
   ...
   * @param  config  Property tree with configuration/options.
   */
  //Armin H. modified to match autocost
  HOVCost(const boost::property_tree::ptree& config, baldr::GraphReader& graphreader);
...
// Constructor
//Armin H. Modified to match autocost
HOVCost::HOVCost(const boost::property_tree::ptree& pt, baldr::GraphReader& graphreader)
    : AutoCost(pt, graphreader) {
}
...
cost_ptr_t CreateHOVCost(const boost::property_tree::ptree& config, baldr::GraphReader& graphreader) {
  return std::make_shared<HOVCost>(config, graphreader);
}
...
AutoCost* make_autocost_from_json(const std::string& property, float testVal) {
  ...
  return new AutoCost(costing_ptree, reader);
}
