...
class TransitCost : public DynamicCost {
 public:
   ...
   */
  //Armin H.
  TransitCost(const boost::property_tree::ptree& pt, baldr::GraphReader& graphreader);
...
//Armin H.
TransitCost::TransitCost(const boost::property_tree::ptree& pt, baldr::GraphReader& graphreader)
    : DynamicCost(pt, graphreader, TravelMode::kPublicTransit) {
...
cost_ptr_t CreateTransitCost(const boost::property_tree::ptree& config, baldr::GraphReader& graphreader) {
  return std::make_shared<TransitCost>(config, graphreader);
}
...
TransitCost* make_transitcost_from_json(const std::string& property, float testVal) {
  ...
  return new TransitCost(costing_ptree, reader);
}
