    ...
    */
    ...
    std::string thor_worker_t::parse_costing(const boost::property_tree::ptree& request) {
      ...
      if (costing == "multimodal" || costing == "transit") {
        ...
        // OLD: mode_costing[0] = get_costing(request, "auto");
        mode_costing[0] = get_costing(request, "auto", reader);
        mode_costing[1] = get_costing(request, "pedestrian", reader);
        mode_costing[2] = get_costing(request, "bicycle", reader);
        mode_costing[3] = get_costing(request, "transit", reader);
        //Armin H.
        mode = valhalla::sif::TravelMode::kPedestrian;
      } else {
        //Armin H. modified costing to accomodate auto costing getting a graph reader reference.
        valhalla::sif::cost_ptr_t cost = get_costing(request, costing, reader);
        //OLD:  valhalla::sif::cost_ptr_t cost = get_costing(request, costing);
        //Armin H.
